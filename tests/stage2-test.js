...
(async () => {
  ...
  assert(await page.locator('#journeyReveal').isHidden(), 'mastery reasoning is exposed before the decision');
  ...
  assert(await page.locator('#journeyReveal').isHidden(), 'Stage 2 answer was disclosed after a miss');
  ...
  await page.click('#journeyCheck');
  assert((await page.locator('#journeyProgressText').textContent()) === '1 / 6', 'Stage 2 mastery was not recorded once');
